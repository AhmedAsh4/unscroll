...
returns exactly one UTF-8 JSON string named `response`. Request and response
frames are at most 262144 UTF-8 bytes. Missing, duplicate, non-string, or
oversized request values receive `invalid_request`.
This bound admits a 65536-byte canonical recovery envelope after worst-case
outer JSON escaping and framing.
