...
A catalog entry has exactly `package_id`, `label`, `user_id`, `launchable`, and
`protected_reason`; `label` is 1–512 ASCII bytes, `launchable` is boolean, and
...
| `device_facts` | empty object | exactly `device_binding`, `capabilities` using the shared shapes |
...
| `icon_stream` | exactly `package_id`, `user_id` | exactly `byte_length`, `mime_type`, `sha256`, `stream_id`; length is 1–1048576, MIME is `image/png`, hash is 64 lowercase hex, stream ID follows shared shape |
...
`read_envelope` and `write_envelope` require equality with the provider's
current device binding or return `device_mismatch`. `write_envelope` validates
