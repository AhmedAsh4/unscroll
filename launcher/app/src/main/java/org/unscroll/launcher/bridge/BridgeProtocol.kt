...
object BridgeProtocol {
    ...
    private const val MAX_REQUEST = 131_072
    ...
    fun parseRequest(input: String): BridgeRequest {
        if (input.toByteArray(Charsets.UTF_8).size > MAX_REQUEST) throw BridgeException(BridgeError.INVALID_REQUEST)
    ...
    fun response(result: Map<String, Any?>): String = Json.write(mapOf("protocol_version" to VERSION, "ok" to true, "result" to result))
