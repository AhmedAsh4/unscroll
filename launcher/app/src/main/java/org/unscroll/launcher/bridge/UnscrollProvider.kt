...
class UnscrollProvider : ContentProvider() {
    ...
    private fun catalog(app: android.content.Context, request: BridgeRequest, userId: Long): String {
        val args = BridgeProtocol.objectArgs(request, setOf("cursor", "page_size", "device_binding")); val size = BridgeProtocol.number(args["page_size"])
        if (size !in 1..100) throw BridgeException(BridgeError.INVALID_REQUEST)
        val binding = binding(args["device_binding"])
        val previous = args["cursor"]
        val snapshot = if (previous == null) {
            val storeFile = java.io.File(app.filesDir, "recovery-v1.json")
            val baseline = PrivateEnvelopeStore(app.filesDir, binding).read()?.baselineLauncherPackage
            ...
            Snapshot(entries, 0, System.currentTimeMillis() + TTL, binding)
        } else synchronized(lock) { cleanup(); pages.remove(BridgeProtocol.boundedString(previous, 32).also { if (!it.matches(Regex("[0-9a-f]{32}"))) throw BridgeException(BridgeError.INVALID_REQUEST) }) }?.takeIf { it.binding == binding } ?: throw BridgeException(BridgeError.NOT_FOUND)
    ...
    private fun icon(app: android.content.Context, request: BridgeRequest, userId: Long): String {
        val args = BridgeProtocol.objectArgs(request, setOf("package_id", "activity_name", "user_id")); val packageId = BridgeProtocol.boundedString(args["package_id"]); val activityName = BridgeProtocol.boundedString(args["activity_name"]); if (BridgeProtocol.number(args["user_id"]) != userId || !packageId.matches(Regex("[A-Za-z_][A-Za-z0-9_$]*(\\.[A-Za-z_][A-Za-z0-9_$]*)+")) || !activityName.matches(Regex("[A-Za-z_][A-Za-z0-9_$]*(\\.[A-Za-z_][A-Za-z0-9_$]*)+"))) throw BridgeException(BridgeError.INVALID_REQUEST)
    ...
    private fun envelope(app: android.content.Context, request: BridgeRequest, write: Boolean): String {
        val args = BridgeProtocol.objectArgs(request, if (write) setOf("device_binding", "envelope") else setOf("device_binding")); val binding = binding(args["device_binding"])
