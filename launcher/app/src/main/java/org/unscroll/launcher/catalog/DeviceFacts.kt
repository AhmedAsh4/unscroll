...
object DeviceFactsProvider {
    ...
    fun trustedBinding(serial: String?, fingerprint: String?): RecoveryDeviceBinding? {
        fun trusted(value: String?) = value?.takeIf { it.isNotBlank() && it.toByteArray(Charsets.UTF_8).size <= 512 && it.all { char -> char.code in 0x20..0x7e } && !it.lowercase().let { text -> text == "unknown" || text == "generic" || "generic" in text || "sdk" in text } }
